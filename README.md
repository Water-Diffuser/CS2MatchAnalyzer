...
The bundle is ~82 MB, almost all of it OpenCV and NumPy, and that is close to
the floor. PyInstaller already zlib-compresses the embedded archive, so shipping
it as a `.gz` or `.xz` saves under 1 MB. Do not reach for `strip=True` either:
it saves about the same 1 MB and corrupts NumPy's vendored OpenBLAS, failing at
startup on Linux and Windows while passing on macOS. Two decisions do keep the
size from being far worse:
